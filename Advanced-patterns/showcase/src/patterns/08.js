    ...
    useLayoutEffect(() => {
        ...
        const triangleBurst = new mojs.Burst({
            ...
            children: {
                ...
                delay: 30,
                speed: 0.2,
                easing: mojs.easing.bezier(0.1, 0.3, 1),
                duration: tlDuration,
...
/**
 * first invocation takes in the function parameters
 * 
 * second invocation takes in whatever arguments are passed in, loops over the function
 * list and invokes the functions with the arguments
 */
...
/**
 * 
 * @returns custom hook for useClapState
 */
const useClapState = (initialState = INITIAL_STATE) => {
    const MAXIMUN_USER_CAP = 50
    const [clapState, setClaptState] = useState(initialState)
    const { count, countTotal } = clapState

    const updateClapState = useCallback(() => {

        setClaptState(({ count, countTotal }) => ({
            isClicked: true,
            count: Math.min(count + 1, MAXIMUN_USER_CAP),
            countTotal: count < MAXIMUN_USER_CAP ? countTotal + 1 : countTotal
        }))
    }, [count, countTotal])
    ...
    const getCounterProps = ({ ...otherProps }) => ({
        count,
        'aria-valuemax': MAXIMUN_USER_CAP,
        'aria-valuemin': 0,
        'aria-valuenow': count,
        ...otherProps
    })
